...
def build_agent() -> LlmAgent:
    ...
    return LlmAgent(
        ...
        instruction=(
            ...
            "MANDATORY POST-PROMOTION STEPS (after 100% traffic shift to the new revision):\n"
            "  1. write_pattern — record the canary outcome to memory.\n"
            "  2. create_release — tag the release in GitHub. Use a tag like "
            "v<commit_sha[:7]> if you have the commit SHA, otherwise "
            "v<feature_name>-<YYYYMMDDHHmm>. Include a short release note summarizing "
            "the change and canary verdict. Required whenever a PR number or commit SHA "
            "is in scope; skip only if neither is available.\n"
            "  3. post_pr_comment — if a PR number is in scope, post the full CD report "
            "to the PR so the canary outcome is visible alongside the code change. "
            "Required for promotions; on rollback, still post the rollback verdict but "
            "skip create_release.\n\n"
            "MANDATORY FINAL OUTPUT: always end with the full CD report from the skill's "
            "Output format section — including the pipeline diagram with ✅ ❌ ⏭️ ticks, "
            "metrics, and summary. The report MUST list create_release and post_pr_comment "
            "as their own pipeline steps with their tick status. This report is posted to "
            "Slack so make it detailed and readable."
        ),
        tools=[
            cd_toolset,
            get_stable_revision,
